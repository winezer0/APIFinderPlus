...
【已完成】实现自定义自动扫描的方法
    【已完成】1、CONF_RECURSE_REQ_HTTP_METHODS 自定义 任意 请求方法列表支持 【每行一种请求方法】
    【已完成】2、CONF_RECURSE_REQ_HTTP_PARAMS 配置禁止自动扫描的URL关键字 如【logout、del】等 防止误删除 
    【已完成】3、CONF_BLACK_RECURSE_REQ_PATH_KEYS 支持自定义请求参数 【一次的请求参数写在一行即可,多行会遍历】
    注意：当前请求的请求头是基于当前URL请求体中动态获取的,后续根据用户需求添加自定义请求头功能。
    
【已完成】实现一个集成的面板，显示所有动态信息
    【已完成】显示当前域名的所有敏感信息、URL、API、等
    【已完成】将详细面板中的对应项都取消显示。

【已完成】表格刷新后的分析问题
    描述：表格刷新以后，排序大概是会变动的，就不知道目前分析到哪里了，如何解决呢？
    解决：目前似乎没有办法解决，除非是动态记录当前行的hash或id信息、这样就需要全部遍历一边才能找到已处理的hash、内存占用上是个问题。
    【已完成方案：添加表格列【已读】 右键可以批量标记已经分析过的目标，可以减少结果数量，便于更新分析。

【已完成】优化加载时间
    【已完成】减少执行插件加载时的数据库创建语句
    【未成功】减少代码的占用 优化jar包大小、清除未调用函数也没小多少
    
    
【已完成】webpack页面的自动自动拼接
    【已完成】实现简单的 webpack 页面js的自动拼接。
    【已忽略】复杂的还没找到规律，暂未=无法实现自动化的 webpack 封装JS内容分析希望大家踊跃提供。
...
4、支持 对webpack的js的简单格式的拼接提取 （限制格式，但准确）
```
### 注意事项
